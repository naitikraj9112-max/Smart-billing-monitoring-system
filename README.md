...
### 2. Local Hosting
Because the dashboard uses modern JavaScript modules (ES Modules), index files cannot be run directly via `file://` protocols in some browsers due to CORS restrictions. The code must be served from a web server:

* **VS Code:** Install the **Live Server** extension, open the project workspace, and click **Go Live**.
* **Python Server:** Run `python -m http.server 8000` in the directory, and navigate to `http://localhost:8000`.
* **Node.js (Npx):** Run `npx live-server` inside the folder.

---

## 🚀 Deployment Guide

This project is built using purely static files, meaning it can be hosted entirely for free on static hosting sites.

### A. Deploying to GitHub Pages

1. **Create GitHub Repository:** Create a new repository on GitHub (e.g., `smart-bill-monitor`).
2. **Initialize Git & Push Workspace:**
   ```bash
   git init
   git add .
   git commit -m "Initialize Smart Bill Monitor Production Build"
   git branch -M main
   git remote add origin https://github.com/your-username/smart-bill-monitor.git
...
3. **Enable GitHub Pages:**
   ...
   - Under **Branch**, select `main` and directory `/ (root)`, then click **Save**.
4. **Access Live Site:** Your dashboard will be live at `https://your-username.github.io/smart-bill-monitor/` within a couple of minutes.

### B. Deploying to Netlify

#### Option 1: Drag-and-Drop (Instant Deployment)
1. Go to [Netlify app](https://app.netlify.com/).
2. Log in or create a free account.
3. Under the **Sites** tab, locate the **"Want to deploy a new site without connecting to Git?"** box at the bottom.
4. Drag and drop your local `smart-bill-monitor` folder directly into the box.
5. Your site is deployed instantly with a custom URL.

#### Option 2: Continuous Deployment from Git
1. Log in to Netlify and click **Add new site** $\rightarrow$ **Import an existing project**.
2. Select your git provider (GitHub) and authorize.
3. Select your `smart-bill-monitor` repository.
4. Leave build settings blank (since this is a static site with no compiler requirement).
5. Click **Deploy Site**. Netlify will automatically build and redeploy the app every time you push code updates to your repo.
